...
class Menu:
    ...
    def createmenu(self, menutype="default"):
        ...
        if menutype == "play":
            ...
            canvas.create_window((self.windowlength / 4) + 100, 500, anchor=CENTER, window=leaderboard_button,
                                 tags="fg")
            canvas.create_window(((3 * self.windowlength) / 4) - 100, 500, anchor=CENTER, window=settings_button,
                                 tags="fg")
            canvas.create_window((self.windowlength / 4) + 100, 650, anchor=CENTER, window=howtoplay_button, tags="fg")
            canvas.create_window(((3 * self.windowlength) / 4) - 100, 650, anchor=CENTER, window=about_button,
                                 tags="fg")
...
class Game:
    ...
    # Enemy spawn
    def enemy_spawn(self, type, spawnx, spawny, movement="none", stopx=0, stopy=0, healthmultiplier=1):
    ...
    def enemybullet(self, type, x, y, shipx=0, shipy=0):
        ...
        if type == "simple":  # Straight laser
            ...
            if type == "special":
                ...
                self.enemybulletspreadlist.append(
                    {"id": bullet, "x": round(movex * 4), "y": round(movey * 4), "counter": spreadcounter})
    ...
    def game_loop(self):
                    ...
                    if enemystats["health"] <= 0:
                        ...
                        try:
                            ...
                            if enemytype == 5:
                                ...
                                if (self.shipstats["level"] < 5) and (
                                        self.expcounter == self.expboundaries[self.expcounter - 1]):
        ...
        for collision in possiblecollisions:
            ...
            if "enemybullet" in tags:
                ...
                self.shipstats[
                    "health"] -= 10 * self.damagemodifier  # Damage calculation, implement variable base damage in the future
        ...
        if self.pausestate or self.bossstate or self.cheatstate:
            ...
            if self.pausestate:
                ...
                quicklevel_button = Button(window, text="Jump to level 5 ship", font=("System", 30),
                                           command=self.level_cheat)
    ...
    def addtoleaderboard(self):
        ...
        back_button = Button(window, text="Back", font=("System", 50),
                             command=Menu(self.windowlength, self.windowheight).createmenu)
    ...
    def startstage(self):
        ...
        elif 10000 <= gametime < 11500:  # Type 4 going across the top from left to right, type 1 going down either side
        ...
        elif 16000 <= gametime < 17000:  # Periodic type 3 moving down the middle
        ...
        elif 22500 <= gametime < 24000:  # 2 type 1 on either side, 2 type 1 in middle, 2 type 2 entering from either side, periodic type 4 from the right
