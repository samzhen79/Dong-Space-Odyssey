...
from tkinter import Tk, PhotoImage, Label, Button, Canvas, CENTER, N, NE, NW, SW
...
class Menu:
	...
	def createmenu(self, menutype="default"):
		...
		if menutype == "play":
			...
			play_button = Button(window, text="Play", font=("Impact", 50),
								 command=lambda menutype="play": self.createmenu(menutype))
			settings_button = Button(window, text="Settings", font=("Impact", 50),
									 command=lambda menutype="settings": self.createmenu(menutype))
			about_button = Button(window, text="About", font=("Impact", 50),
...
class Game:
	...
	def enemybullet(self, type, x, y, shipx=0, shipy=0):
		...
		if type == "simple":  # Straight laser
			...
			canvas.create_image(x, y, anchor=N, image=enemylaserstraight_image,
			...
			bullet = canvas.create_image(x, y, anchor=N, image=enemylaserround_image,
			...
			bulletx0, bullety0, bulletx1, bullety1 = bbox(bullet["id"])
			bulletx = (bulletx0 + bulletx1) / 2
			bullety = (bullety0 + bullety1) / 2
			directDist = sqrt(((shipx - bulletx) ** 2) + ((shipy - bullety) ** 2))
			...
			bullet = canvas.create_image(x, y, anchor=N, image=enemylaserroundcross_image,
	...
	def saveonclose(self):
		...
		self.savestate.write(" " + str(self.gametime) + "~" + json.dumps(self.shipstats) + "~" + json.dumps(
			self.enemylist) + "~" + json.dumps(self.enemybulletspeciallist) + "~" + json.dumps(
			self.enemybulletspreadlist) + "~" + str(self.score) + "\n")
	...
	def game_loop(self):
		...
		#These are all for the purpose of performance (used in loops)
		bbox = canvas.bbox
		move = canvas.move
		createimage = canvas.create_image
		gettags = canvas.gettags
		...
		if self.shoot == True:
			...
			if not (self.attackinterval % (10)):  # % x indicates fire rate
				...
				createimage(x0 + 90, y0 + 180, image=playerlaserstraight_image,
							tag=("fg", "playerbullet", "game", "gameimage"))
				createimage(x1 - 90, y0 + 180, image=playerlaserstraight_image,
							tag=("fg", "playerbullet", "game", "gameimage"))
		...
		for enemy in self.enemylist:
			...
			enemyid = enemy["id"]
			enemyx0, enemyy0, enemyx1, enemyy1 = bbox(enemyid)
			# Movement
			if enemymovement == "forward":
				move(enemyid, 0, enemyspeed)
			elif enemymovement == "right":
				move(enemyid, enemyspeed, 0)
			elif enemymovement == "left":
				move(enemyid, -enemyspeed, 0)
			elif enemymovement == "diagonalright":
				move(enemyid, enemyspeed * (sqrt(2) / 2), enemyspeed * (sqrt(2) / 2))
				...
				directDist = sqrt(((enemystats["stopx"] - enemyx) ** 2) + ((enemystats["stopy"] - enemyy) ** 2))
				if directDist > 0:
					movex = (enemystats["stopx"] - enemyx) / directDist
					movey = (enemystats["stopy"] - enemyy) / directDist
					move(enemy, movex * enemyspeed, movey * enemyspeed)
			...
			for collision in canvas.find_overlapping(enemyx0, enemyy0, enemyx1, enemyy1):
				...
				if "playerbullet" in gettags(collision):
		...
		move("enemybulletstraight", 0, 10)

		move("enemybulletspread", 0, 5)
		for bullet in self.enemybulletspreadlist:  # Loop to spawn radiating bullets around spread type bullets
			...
			if not (bulletcounter % (50)) and (bulletcounter != 0):
				...
				for i in range(8):
					movex = dist * sin(pi * (i / 4))
					movey = dist * cos(pi * (i / 4))
					bulletcreate = createimage(bulletx + movex, bullety + movey, anchor=N, image=enemylaserround_image,
											   tag=("fg", "enemybullet", "enemybulletspecial", "game", "gameimage"))
		...
		for bullet in self.enemybulletspeciallist:  # For bullets that don't go straight down
			...
			move(bullet["id"], bullet["x"], bullet["y"])

		# Player Collision and Damage
		hitboxx0, hitboxy0, hitboxx1, hitboxy1 = bbox(self.hitbox)
		...
		for collision in possiblecollisions:
			...
			if "enemybullet" in gettags(collision):
		...
		if self.gametime == 20:
			...
			self.enemy_spawn(3, 275, 200, "stop", 225, 200)
			self.enemy_spawn(3, 325, 200, "stop", 225, 200)
			self.enemy_spawn(3, 375, 200, "stop", 225, 200)
			self.enemy_spawn(3, 425, 200, "stop", 225, 200)
			self.enemy_spawn(3, 475, 200, "stop", 225, 200)
			self.enemy_spawn(3, 525, 200, "stop", 225, 200)
			self.enemy_spawn(3, 575, 200, "stop", 225, 200)
			self.enemy_spawn(3, 625, 200, "stop", 225, 200)
			self.enemy_spawn(3, 675, 200, "stop", 225, 200)
			self.enemy_spawn(3, 725, 200, "stop", 225, 200)
		...
		if self.pausestate or self.bossstate:
			...
			if self.pausestate:
				...
				createimage(0, 0, anchor=NW, image=bosskey_image, tags=("fg", "pausebutton"))
